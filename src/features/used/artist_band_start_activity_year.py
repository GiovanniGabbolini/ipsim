...
@cached_feature
@timing_feature
def artist_band_start_activity_year(artist_musicbrainz_id) -> 'year':
    """For bands, extracts the year the band was formed, None otherwise.
    ...
    if artist_type == 'Group':
        ...
        try:
            start_activity_year = artist['artist']['life-span']['begin']
        except KeyError:
            logging.getLogger('root.features').warning(
                f"Band {artist_musicbrainz_id} do not have known start activity date")
            return None
        ...
        if re.match(r"^\d{4}$", start_activity_year):
            ...
            logging.getLogger('root.features').warning(
                f"Start activity year {start_activity_year} does not match pattern")


if __name__ == "__main__":
    array_feature(artist_band_start_activity_year, mp=False)
